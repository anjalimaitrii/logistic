...
import { CITY_TO_STATE } from "@/lib/africaLocations";
...
export default function AdminClients() {
  // Same country/province/city lists the booking forms use, so a company address
  // names a place the same way a trip endpoint does.
  const {
  ...
  const handleSubmit = async () => {
    ...
    try {
      const { contactCode, ...clientFields } = formData;
      await clientService.create({
        ...clientFields,
        // Stored as one string, the way every other screen reads it. The dial
        // code is a control on the form, not a field on the record.
        contact: joinDialCode(contactCode, formData.contact),
        status: "Active",
  ...
  const handleCompanySubmit = async () => {
    ...
    const payload = {
      ...
      tpinNumber: companyForm.tpinNumber,
      address: {
    ...
    try {
      ...
      setCompanyForm({
        companyName: "", tpinNumber: "", street: "", country: "", state: "", city: "",
        billingName: ""
      });
  ...
  const handleAddClientClick = (company: any) => {
    ...
    setFormData({ name: "", email: "", contactCode: DEFAULT_DIAL_CODE, contact: "", designation: "", password: "" }); // Reset
    setFormStep(1);
    setModalOpen(true);
  };
  ...
  /**
   * Removing a company takes its client accounts with it — they are that
   * company's people, and an account under a company that no longer exists has
   * nothing to log in to. Their trade is left alone, with both names written onto
   * the bookings first.
   */
  const handleDeleteCompany = async (company: any) => {
    ...
    let warning = "";
    try {
      const usage = await companyService.usage(company._id);
      if (usage.total > 0) {
        warning =
          `${BREAK}${name} has ${usage.summary}.\n` +
          `The client accounts are removed too. Those records are kept and will ` +
          `still show the client and company name.`;
      }
    } catch {
      warning = `${BREAK}(Could not check what this company is attached to.)`;
    }
    if (!confirm(`Delete ${name}? This cannot be undone.${warning}`)) return;

    try {
      await companyService.remove(company._id);
      await fetchData();
    } catch (error) {
  ...
  /**
   * Removing a client account. Their bookings, invoices and payments are NOT
   * deleted — they stay as the record of what happened and simply lose the name,
   * reading as "Direct Client" afterwards. That cannot be undone, so the counts
   * come off the server first and go into the confirmation verbatim.
   */
  const handleDeleteClient = async (staff: any) => {
    const BREAK = "\n\n";
    let warning = "";
    try {
      const usage = await clientService.usage(staff._id);
      if (usage.total > 0) {
        warning =
          `${BREAK}${staff.name} is attached to ${usage.summary}.\n` +
          `Those records are kept, but they will lose this client's name and show as "Direct Client".`;
      }
    } catch {
      // Counts are a courtesy; not being able to read them must not block ops.
      warning = `${BREAK}(Could not check what this client is attached to.)`;
    }
    if (!confirm(`Delete ${staff.name}? This cannot be undone.${warning}`)) return;

    try {
      await clientService.remove(staff._id);
      // The open directory is a snapshot — drop the row from it too, or the card
      // sits there until the modal is reopened.
      setSelectedCompanyForView((c: any) =>
        c ? { ...c, clients: (c.clients || []).filter((x: any) => x._id !== staff._id) } : c
      );
      await fetchData();
    } catch (error) {
                  ...
                  <div className="space-y-2.5 mb-2 bg-neutral-50/40 rounded-xl p-3.5 border border-neutral-100/30">
                    ...
                    <div className="grid grid-cols-2 gap-2">
                      {viewType === 'companies' ? (
                        <>
                          <div>
                            <p className="text-[8px] font-bold text-neutral-300 uppercase tracking-wider mb-0.5">TPIN</p>
                            <p className="text-[10px] font-bold text-slate-600 truncate">{item.tpinNumber || "N/A"}</p>
                          </div>
                          <div>
                    ...
                    {viewType === 'companies' ? (
                      <>
                        ...
                        <button
                          ...
                          title={`Delete ${item.companyName || item.name}`}
                          className="flex items-center justify-center px-3 py-2 rounded-lg bg-neutral-50 text-neutral-300 hover:text-rose-600 hover:bg-rose-50 transition-all cursor-pointer"
                        >
              ...
              {formStep === 1 ? (
                <div className="space-y-4">
                  ...
                  <div className="space-y-1.5">
                    ...
                    <PhoneInput
                      code={formData.contactCode}
                      value={formData.contact}
                      placeholder="771 234 567"
                      onCodeChange={(code) => setFormData({ ...formData, contactCode: code })}
                      onValueChange={(local) => setFormData({ ...formData, contact: local })}
                    />
                  </div>
                  <div className="space-y-1.5">
          ...
          >
            ...
            <div className="px-7 py-6 space-y-5 flex-1 overflow-y-auto custom-scrollbar">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                ...
                <div className="space-y-1.5">
                  <label className="text-[11px] font-semibold text-slate-500 uppercase tracking-widest">TPIN Number</label>
                  <input type="text" placeholder="1002345678" value={companyForm.tpinNumber} onChange={(e) => setCompanyForm({ ...companyForm, tpinNumber: e.target.value })} className={inputClass} />
                </div>
              </div>
              ...
              <div className="space-y-3">
                ...
                <ComboBox
                  ...
                  onChange={(name) => setCompanyForm({ ...companyForm, country: name, state: "", city: "" })}
                ...
                <div className="grid grid-cols-2 gap-3">
                  <ComboBox
                    ...
                    options={stateOptionsFor(companyForm.country)}
                    placeholder="State / Province"
                    disabled={!companyForm.country}
                    className={inputClass}
                    onChange={(name) => setCompanyForm({ ...companyForm, state: name, city: "" })}
                    onCreate={(name) => createLocation("state", name, { country: companyForm.country })}
                    onDelete={deleteLocation}
                  />
                  <ComboBox
                    value={companyForm.city}
                    options={cityOptionsFor(companyForm.country)}
                    placeholder="City"
                    disabled={!companyForm.country}
                    className={inputClass}
                    onChange={(name) => {
                      // Shipped towns know their province; one an operator added
                      // keeps whatever was chosen above.
                      const autoState = companyForm.country ? (CITY_TO_STATE[companyForm.country]?.[name] || "") : "";
                      setCompanyForm({ ...companyForm, city: name, state: autoState || companyForm.state });
                    }}
                    onCreate={(name) => createLocation("city", name, { country: companyForm.country, state: companyForm.state })}
                    onDelete={deleteLocation}
                  />
                </div>
              </div>
                  ...
                  <div key={staff._id} className="bg-white p-5 rounded-2xl border border-neutral-100 shadow-sm hover:border-primary/20 transition-all group">
                    <div className="flex items-start justify-between mb-4">
                      ...
                      <div className="flex items-center gap-2">
                        ...
                        <button
                          onClick={() => handleDeleteClient(staff)}
                          title={`Delete ${staff.name}`}
                          className="p-1.5 rounded-lg text-neutral-300 hover:text-rose-600 hover:bg-rose-50 transition-all cursor-pointer"
                        >
